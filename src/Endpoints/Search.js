...
app.get('/api/search', async (req, res) => {
    try {
        ...
        if (mode) {
            const modeColMap = {
                osu: 'mode_osu_count',
                taiko: 'mode_taiko_count',
                fruits: 'mode_fruits_count',
                catch: 'mode_fruits_count',
                mania: 'mode_mania_count'
            };
            ...
            if (modeClauses.length) {
                where.push(`(${modeClauses.join(' OR ')})`);
            }
        }

        // Video filter
        if (video === 'true')  {
            where.push(`video = true`);
        }
        if (video === 'false') {
            where.push(`(video = false OR video IS NULL)`);
        }

        // BPM range (beatmapset level)
        if (bpm_min !== undefined && bpm_min !== '') {
        ...
        tryBmFilter('ar', ar_min, '>=');
        tryBmFilter('ar', ar_max, '<=');
        tryBmFilter('cs', cs_min, '>=');
        tryBmFilter('cs', cs_max, '<=');
        tryBmFilter('accuracy', od_min, '>=');
        tryBmFilter('accuracy', od_max, '<=');
        tryBmFilter('drain', hp_min, '>=');
        tryBmFilter('drain', hp_max, '<=');
        tryBmFilter('total_length', length_min, '>=');
        tryBmFilter('total_length', length_max, '<=');

        // Exact beatmap ID
        if (map_id) {
            const mid = parseInt(map_id, 10);
            if (!isNaN(mid)) {
                bmFilters.push(`bm.id = $${idx}`);
                values.push(mid);
                idx++;
            }
        }
        ...
        if (rows.length > 0) {
            ...
            const bmRes = await pool.query(
                `SELECT * FROM ${process.env.TABLE_BEATMAP} WHERE beatmapset_id = ANY($1)`,
                [ids]
            );
            const bmMap = {};
            for (const bm of bmRes.rows) {
                if (!bmMap[bm.beatmapset_id])
                    bmMap[bm.beatmapset_id] = [];
                bmMap[bm.beatmapset_id].push(bm);
            }
            beatmapsets = rows.map(r => serializeBeatmapset(r, bmMap[r.id] || []));
        }

        const response = { beatmapsets };
        await redis.set(cacheKey, JSON.stringify(response), 'EX', process.env.REDIS_CACHE_TIME);
        res.json(response);
    } catch (err) {
